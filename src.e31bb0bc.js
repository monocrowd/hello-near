...
var CONTRACT_NAME = "dev-1637338348857-42879230422474" || 'greet.happybits.testnet';
  ...
  switch (env) {
    ...
    case 'local':
      return {
        ...
        keyPath: "".concat("/Users/cksac", "/.near/validator_key.json"),
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            _event$target$element = event.target.elements, fieldset = _event$target$element.fieldset, greeting = _event$target$element.greeting; // update the greeting in the UI

            _context.next = 4;
            return fetchGreeting(greeting.value);
...
function signedInFlow() {
  document.querySelector('#signed-in-flow').style.display = 'block'; // populate links in the notification box
...
function fetchGreeting(_x2) {
  return _fetchGreeting.apply(this, arguments);
} // `nearInitPromise` gets called on page load


function _fetchGreeting() {
  _fetchGreeting = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(name) {
...
function TextAnimation(textGeometry) {
  ...
  for (i = 0, i2 = 0, i3 = 0, i4 = 0; i < faceCount; i++, i2 += 6, i3 += 9, i4 += 12) {
    ...
    var dirY = centroid.y > 0 ? 1 : -1; // animation
  ...
  var material = new THREE.BAS.BasicAnimationMaterial({
    ...
    shaderVertexInit: ['float tDelay = aAnimation.x;', 'float tDuration = aAnimation.y;', 'float tTime = clamp(uTime - tDelay, 0.0, tDuration);', 'float tProgress = tTime / tDuration;'],
    shaderTransformPosition: ['vec3 tPosition = transformed;', 'tPosition *= 1.0 - tProgress;', 'tPosition += cubicBezier(transformed, aControl0, aControl1, aEndPosition, tProgress);', 'transformed = tPosition;']
  }, {
    diffuse: 0x000000
  });
...
function THREERoot(params) {
  ...
  var threeNode = document.getElementById("three-container");
